        ...
        <div style={{ maxWidth: 460, margin: "32px auto 0" }}>
          {!submitted ? (
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
              ...
              />
              <button onClick={handleSubscribe} disabled={nlLoading || !email.trim()} style={{
                background: nlLoading ? "var(--bg-input)" : "linear-gradient(135deg, var(--accent), var(--accent-dark))", border: "none", borderRadius: 10,
                padding: "14px 32px", color: nlLoading ? "var(--text-muted)" : "var(--bg-main)", fontFamily: "'DM Sans', sans-serif", fontWeight: 700,
                fontSize: 15, cursor: nlLoading ? "default" : "pointer", whiteSpace: "nowrap",
              }}>{nlLoading ? "Subscribing..." : "Subscribe Free"}</button>
            </div>
          ) : (
            <div style={{
              ...
              <div style={{ color: "var(--accent)", fontFamily: "'DM Sans', sans-serif", fontSize: 16, fontWeight: 600 }}>{nlMsg || "You're in!"}</div>
              <div style={{ color: "var(--text-secondary)", fontSize: 13, marginTop: 4 }}>Welcome aboard! Stay tuned for our first issue.</div>
            </div>
          )}
